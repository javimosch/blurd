-- blurd storage schema, PostgreSQL.
--
-- The SQLite file (schema.sql) is the reference; this is its Postgres
-- counterpart. They must declare the same tables, columns and indexes --
-- asserted by tests/schema_drift.py, because two hand-maintained schemas
-- diverge silently otherwise.
--
-- Deliberate choices:
--   * timestamps stay TEXT (RFC3339, UTC). Keeping them textual means the
--     cursors, comparisons and stored values are byte-identical across
--     engines, and the portable SQL in db.py needs no date handling at all.
--   * the INTEGER flags (needs_review, cached, force) stay integers rather
--     than becoming BOOLEAN, so the same statements and the same values work
--     on both.
--   * AUTOINCREMENT becomes an identity column; BLOB becomes BYTEA.
--   * SQLite REAL becomes DOUBLE PRECISION, *not* Postgres REAL. SQLite's REAL
--     is 8-byte IEEE; Postgres REAL is 4-byte. Keeping the name would silently
--     halve the precision -- and it broke keyset pagination, because a cursor
--     value round-tripped through float8 compares equal to neither the float4
--     column nor anything else, so the boundary row was matched by neither the
--     `<` branch nor the `=` branch and appeared on both pages.
