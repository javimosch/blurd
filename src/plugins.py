"""Optional pro/commercial plugin seam.
...
OSS code never imports plugin internals; the seam is one dict and one hook.
...
def load(cfg) -> Optional[Any]:
    ...
    The plugin decides for itself whether it is licensed/enabled; OSS only
    checks importability and calls install()."""
    try:
